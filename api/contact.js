    ...
    async fetch(request) {
        ...
        try {
            if (!process.env.RESEND_API_KEY) {
                return json({ ok: false, message: "RESEND_API_KEY não configurada." }, { status: 500 });
            }

            if (!process.env.CONTACT_TO_EMAIL || !process.env.CONTACT_FROM_EMAIL) {
                return json({ ok: false, message: "E-mails do contato não configurados." }, { status: 500 });
            }
            ...
            if (!isValidEmail(email)) {
                return json({ ok: false, message: "Informe um e-mail válido." }, { status: 400 });
            }
        ...
        `
            ...
            if (error) {
                ...
                return json(
                    { ok: false, message: error.message || "Não foi possível enviar seu contato agora." },
                    { status: 500 }
                );
            }
            ...
            return json(
                { ok: false, message: error?.message || "Erro interno ao processar o envio." },
                { status: 500 }
            );
        }
    }
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!process.env.CONTACT_TO_EMAIL || !process.env.CONTACT_FROM_EMAIL) {
            return res.status(500).json({
                ok: false,
                message: "E-mails do contato não configurados.",
            });
        }
      ...
      `,
        ...
        if (error) {
            ...
            return res.status(500).json({
                ok: false,
                message: error.message || "Não foi possível enviar seu contato agora.",
            });
        }
        ...
        return res.status(500).json({
            ok: false,
            message: error?.message || "Erro interno ao processar o envio.",
        });
    }
}